...
(function () {
  ...
  var SCROLL_GAP = 40;   // air left above a section's content after a jump
  ...
  /* ---- in-page navigation ----------------------------------------------
     ...
     destination at click time, then web fonts swap and lazy images resolve
     ABOVE the target, the page grows, and the landing drifts. So we compute
     the destination ourselves and re-check it once the page settles.
  ...
  /* where a section should come to rest: its first real content, SCROLL_GAP
     below the top of the viewport (not its border box, which would leave the
     section's whole top padding as dead space) */
  function destinationFor(target) {
    var anchor = target.querySelector(".container > *") || target;
    var top = anchor.getBoundingClientRect().top + window.pageYOffset - SCROLL_GAP;
  ...
  function scrollToTarget(target) {
    ...
    [800, 1400, 2200].forEach(function (delay) {
      setTimeout(function () {
        ...
        if (Math.abs(window.pageYOffset - want) > 4) {
