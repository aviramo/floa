...
/* --- and a verse that ends on the chord it begins on ------------------------
   THE CEILING ON THE WAIT ABOVE, and the readings below are a real recording's
   because nothing shorter reproduced it.

   A great many verses end on the chord they start on, and a repeat puts those
   two identical chords side by side (see songSpans in app.js). Nothing in the
   sound can ever separate them: the mark sits on the first until the chord
   AFTER them arrives and pulls it across both at once. So crossing the second
   one is paid for entirely in the bonus for standing still, seven tenths a
   reading, with no difference in score anywhere to earn it back, and that is
   the one place in the song where the wait is charged at full price.

   Which is why the wait cannot be long. At four readings and above this
   recording never crossed the seam: the mark stopped at the end of the first
   verse and stood there for the remaining twenty four seconds of playing. The
   line below is the whole of that stretch, sixteen seconds of it, one row per
   three readings the way the trace is written (see traceOn in app.js), in the
   song's own order of kinds: Am, C, Em, G, C/B. */
{
  const verse = ["Am", "C", "Em", "Am", "C", "C/B", "Am"];
  /* the verse twice, which is what `|:` ... `:|2` hands the follower */
  const f = F.make([].concat(verse, verse), [0, 7]);
  /* standing on the last chord of the first time through */
  f.put(6);

  const seam = [
    [0.77,0.51,0.47,0.51,0.30], [0.92,0.56,0.45,0.56,0.21], [0.83,0.59,0.52,0.59,0.21], [0.74,0.55,0.56,0.55,0.17],
    [0.75,0.46,0.40,0.46,0.25], [0.71,0.52,0.54,0.52,0.25], [0.45,0.49,0.63,0.49,0.29], [0.64,0.50,0.52,0.50,0.20],
    [0.66,0.50,0.61,0.50,0.14], [0.65,0.48,0.59,0.48,0.12], [0.78,0.44,0.43,0.44,0.13], [0.77,0.44,0.44,0.44,0.17],
    [0.80,0.40,0.40,0.40,0.19], [0.82,0.40,0.39,0.40,0.15], [0.67,0.44,0.45,0.44,0.14], [0.67,0.38,0.47,0.38,0.16],
    [0.68,0.41,0.52,0.41,0.15], [0.70,0.42,0.45,0.42,0.33], [0.78,0.38,0.38,0.38,0.17], [0.64,0.52,0.50,0.52,0.15],
    [0.86,0.45,0.41,0.45,0.19], [0.71,0.49,0.49,0.49,0.13], [0.80,0.48,0.44,0.48,0.13], [0.80,0.45,0.42,0.45,0.14],
    [0.76,0.47,0.49,0.47,0.25], [0.72,0.31,0.27,0.31,0.21], [0.87,0.44,0.40,0.44,0.16], [0.74,0.47,0.42,0.47,0.15],
    [0.87,0.44,0.38,0.44,0.18], [0.85,0.45,0.41,0.45,0.15], [0.65,0.54,0.52,0.54,0.39], [0.67,0.55,0.54,0.55,0.19],
    [0.65,0.59,0.59,0.59,0.33], [0.77,0.41,0.40,0.41,0.18], [0.80,0.40,0.40,0.40,0.17], [0.78,0.42,0.41,0.42,0.14],
    [0.74,0.36,0.35,0.36,0.14], [0.72,0.35,0.37,0.35,0.22], [0.79,0.52,0.45,0.52,0.22], [0.87,0.52,0.43,0.52,0.20],
    [0.82,0.58,0.52,0.58,0.26], [0.69,0.53,0.49,0.53,0.20], [0.87,0.56,0.46,0.56,0.27], [0.77,0.53,0.45,0.53,0.31],
    [0.58,0.61,0.71,0.61,0.26], [0.57,0.61,0.75,0.61,0.35], [0.63,0.55,0.57,0.55,0.27], [0.70,0.45,0.46,0.45,0.19],
    [0.68,0.51,0.52,0.51,0.18], [0.75,0.44,0.44,0.44,0.12], [0.74,0.42,0.45,0.42,0.17], [0.74,0.48,0.47,0.48,0.24],
    [0.63,0.58,0.50,0.58,0.24], [0.82,0.63,0.53,0.63,0.22], [0.68,0.52,0.55,0.52,0.33], [0.63,0.36,0.43,0.36,0.45],
    [0.66,0.47,0.52,0.47,0.41], [0.67,0.43,0.44,0.43,0.23], [0.73,0.47,0.47,0.47,0.25], [0.63,0.53,0.54,0.53,0.35],
    [0.71,0.45,0.45,0.45,0.24], [0.71,0.31,0.32,0.31,0.24], [0.63,0.50,0.46,0.50,0.24], [0.82,0.44,0.38,0.44,0.27],
    [0.63,0.55,0.68,0.55,0.22], [0.58,0.49,0.63,0.49,0.19], [0.65,0.58,0.71,0.58,0.28], [0.57,0.59,0.67,0.59,0.53],
    [0.69,0.54,0.64,0.54,0.47], [0.73,0.53,0.65,0.53,0.49], [0.70,0.50,0.55,0.50,0.26], [0.77,0.43,0.49,0.43,0.27],
    [0.74,0.41,0.47,0.41,0.24], [0.73,0.41,0.48,0.41,0.24], [0.72,0.40,0.49,0.40,0.22], [0.73,0.40,0.48,0.40,0.27],
    [0.80,0.47,0.49,0.47,0.20], [0.83,0.42,0.46,0.42,0.26], [0.81,0.51,0.54,0.51,0.21], [0.78,0.52,0.57,0.52,0.27],
    [0.77,0.46,0.53,0.46,0.29], [0.51,0.55,0.64,0.55,0.47], [0.48,0.61,0.69,0.61,0.58], [0.68,0.42,0.51,0.42,0.43],
    [0.81,0.49,0.54,0.49,0.27], [0.69,0.53,0.58,0.53,0.36], [0.66,0.57,0.57,0.57,0.22], [0.82,0.59,0.57,0.59,0.25],
    [0.80,0.57,0.55,0.57,0.21], [0.76,0.54,0.54,0.54,0.21], [0.79,0.56,0.51,0.56,0.19], [0.63,0.60,0.53,0.60,0.17],
    [0.65,0.61,0.62,0.61,0.17], [0.74,0.57,0.49,0.57,0.16], [0.72,0.56,0.50,0.56,0.28], [0.78,0.56,0.48,0.56,0.17],
    [0.69,0.49,0.47,0.49,0.28], [0.51,0.54,0.66,0.54,0.14], [0.56,0.64,0.70,0.64,0.24], [0.54,0.71,0.83,0.71,0.32],
    [0.45,0.67,0.66,0.67,0.56], [0.48,0.73,0.82,0.73,0.53], [0.54,0.79,0.86,0.79,0.49], [0.53,0.75,0.83,0.75,0.50],
    [0.55,0.72,0.77,0.72,0.43], [0.54,0.70,0.75,0.70,0.45], [0.54,0.83,0.72,0.71,0.37], [0.54,0.78,0.85,0.78,0.45],
    [0.54,0.79,0.86,0.79,0.41], [0.54,0.77,0.85,0.77,0.38], [0.45,0.75,0.83,0.75,0.49], [0.43,0.72,0.81,0.72,0.48],
    [0.44,0.58,0.72,0.58,0.40], [0.48,0.71,0.85,0.71,0.43], [0.49,0.73,0.87,0.73,0.52], [0.39,0.62,0.72,0.62,0.47],
    [0.49,0.73,0.79,0.73,0.37], [0.51,0.78,0.85,0.78,0.46], [0.39,0.72,0.84,0.72,0.57], [0.49,0.77,0.81,0.77,0.48],
    [0.53,0.91,0.77,0.79,0.43], [0.43,0.66,0.66,0.64,0.49], [0.46,0.73,0.85,0.73,0.50], [0.45,0.71,0.72,0.71,0.45],
    [0.46,0.74,0.85,0.74,0.44], [0.47,0.72,0.76,0.72,0.42], [0.48,0.72,0.83,0.72,0.36], [0.34,0.48,0.67,0.48,0.54],
    [0.34,0.49,0.69,0.49,0.54], [0.38,0.58,0.65,0.58,0.47], [0.27,0.58,0.86,0.58,0.63]
  ];
  for (const k of seam) for (let i = 0; i < 3; i++) f.step(k);

  eq("a verse coming round on the chord it ended on is still crossed", f.where() > 6, true);
}
